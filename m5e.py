...
class M5e:
    ...
    def __init__(self, portINT=-1, portSTR='/dev/ttyS0', baudrate=9600, 
        TXport=1, RXport=1, readPwr=2300, protocol='GEN2', compact=True, verbosity=False):
        if portINT != -1:
    ...
    def ReadMultiTag(self):
		...
		#a=self.readEntireTagBuffer();
		#print a
		return self.readEntireTagBuffer();
    ...
    def _ReceiveResponse(self):
        ...
        if printAll==True:
            print "start = %s" % self.ReturnHexString(start)
        if start != '\xFF':
            time.sleep(5)
            self.ser.flushInput()
        ...
        if printAll==True:
            print "length = %s" % self.ReturnHexString(length)
        if len(length) != 1:
            time.sleep(5)
            self.ser.flushInput()
        ...
        if printAll==True:
            print "command = %s" % self.ReturnHexString(command)
        if len(command) != 1:
            time.sleep(5)
            self.ser.flushInput()
        ...
        if printAll==True:
            print "status = %s" % self.ReturnHexString(status)
        if len(status) != 2:
            time.sleep(5)
            self.ser.flushInput()
        ...
        if printAll==True:
            print "data = %s" % self.ReturnHexString(data)
        ...
        CRC = self.ser.read(2)
        if printAll==True:
            print "CRC = %s" % self.ReturnHexString(CRC)
        if len(CRC) != 2:
            time.sleep(5)
            self.ser.flushInput()
